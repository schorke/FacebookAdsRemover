/* ----------------------------------------------------------------------------------
 * Author: Kevin Schorkops;
 * Written: 13/08/16
 * Last Updated: 13/08/16
 * Description: Removes ads in the Facebook news feed.
 *
 * Dependencies: jquery
 * ----------------------------------------------------------------------------------
 */
...
function removeAds(idStart) {
  ...
  if (adDivs.length > 0) {
   ...
   adDivs[0].parentNode.removeChild(adDivs[0]);
    ...
    prepend += "Ceci est une pub et\n";
    prepend += "VA TE FAIRE FOUTRE";
    prepend += "</span>";
    prepend += "</div>";
